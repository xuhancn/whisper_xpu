...
if(ONEDNN_STATIC)
    ...
    if(DEFINED ENV{DNNLROOT})
        ...
        # Derive from ONEAPI_ROOT or CMPLR_ROOT (set by oneAPI setvars)
        set(_oneapi_root "")
        if(DEFINED ENV{ONEAPI_ROOT})
            set(_oneapi_root "$ENV{ONEAPI_ROOT}")
        elseif(DEFINED ENV{CMPLR_ROOT})
            get_filename_component(_oneapi_root "$ENV{CMPLR_ROOT}/../.." ABSOLUTE)
        endif()
        if(_oneapi_root)
        ...
        # Fallback: well-known default paths
        if(NOT DNNLROOT)
    ...
    if(nodnnl)
        ...
        if(CMAKE_SYSTEM_NAME MATCHES "Windows")
            # Derive OpenCL path from environment (set by oneAPI setvars.bat)
            if(DEFINED ENV{CMPLR_ROOT})
                set(_ocl_root "$ENV{CMPLR_ROOT}")
            elseif(DEFINED SYCL_COMPILER)
                get_filename_component(_ocl_root "${SYCL_COMPILER}/../.." ABSOLUTE)
            else()
                set(_ocl_root "")
            endif()
            if(_ocl_root AND EXISTS "${_ocl_root}/include/CL/opencl.h")
