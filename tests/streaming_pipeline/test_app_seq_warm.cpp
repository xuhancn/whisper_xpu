...
int main(int argc, char** argv) {
    ...
    fprintf(stderr, "=== App-sequence repro (WITH transcribe_file warmup, like test_pipeline) ===\n");
    ...
    // ── WARMUP: transcribe_file on the main thread, like test_pipeline does ──
    fprintf(stderr, "WARMUP: transcribe_file on main thread...\n");
    try {
        auto r = eng.transcribe_file(audio_path);
        fprintf(stderr, "warmup: %zu chars, %d segs, %.0fms\n",
